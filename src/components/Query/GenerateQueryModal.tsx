...
export const GenerateQueryModal: React.FC<GenerateQueryModalProps> = ({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const fieldsToUse = selectedFields.length > 0 ? selectedFields : lookupFields;

      const payload = {
        lookupName: selectedLookup,
        lookupFieldsUsed: fieldsToUse,
      ...
      <div className="space-y-4">
        ...
        {selectedLookup && selectedLookup !== 'None' && (
          <div className="w-full">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Select specific fields to use in the query (if none selected, all fields will be used)
            </p>
          </div>
        )}
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3">
          <div className="flex items-start">
            ...
            <div>
              <p className="text-sm font-medium text-yellow-800">AI-Generated Content</p>
              <p className="text-sm text-yellow-700 mt-1">
                The generated query may not be completely accurate. Please review and test the query before using it in production.
              </p>
            </div>
          </div>
        </div>
