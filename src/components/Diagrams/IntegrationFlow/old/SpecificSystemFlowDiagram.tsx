...
const SpecificSystemFlowDiagram: React.FC<SpecificSystemFlowDiagramProps> = ({
  ...
  // Color scales - matching the original HTML
  ...
  // Tooltip handlers - matching original HTML
  const handleNodeMouseOver = useCallback((event: any, d: any) => {
    ...
    tooltip.html(`<strong>${d.name} (${d.id})</strong><br/>Type: ${d.type}<br/>Criticality: ${d.criticality}<br/>Internet Facing: ${d.internetFacing || 'N/A'}<br/>Environment: ${d.environment || 'N/A'}${clickText}`)
  ...
  // Process data for Sankey - matching original HTML
  const processDataForSankey = useCallback((data: any) => {
    ...
    const links = data.links.map((l: any) => ({...l, value: l.value || 1}));
    return { nodes: data.nodes, links };
  }, []);

  // Create Sankey diagram - matching original HTML implementation
  const createSankey = useCallback((data: SystemDependencyDiagramData) => {
    ...
    const pinnedSystemCode = data.metadata.code;

    // Create sankey generator - matching original HTML configuration
    const sankeyGenerator = sankey()
      ...
      .nodeSort((a: any, b: any) => {
        // Matching the original HTML sorting logic
        const allLinks = (node: any) => [...(node.sourceLinks||[]), ...(node.targetLinks||[])];

        const touchesMW = (node: any) =>
          integrationMiddleware.includes(node.id) ||
          allLinks(node).some((l: any) =>
            integrationMiddleware.includes(l.source.id) ||
            integrationMiddleware.includes(l.target.id)
          );

        const directToMain = (node: any) =>
          allLinks(node).some((l: any) =>
            (l.source.id === pinnedSystemCode && l.target.id !== pinnedSystemCode) ||
            (l.target.id === pinnedSystemCode && l.source.id !== pinnedSystemCode)
          );

        // MW-connected nodes first
        const aMW = touchesMW(a), bMW = touchesMW(b);
        if (aMW !== bMW) return aMW ? -1 : 1;

        // Sort by middleware base alphabetically
        const getBase = (node: any) => {
          if (integrationMiddleware.includes(node.id)) {
            return node.id.split('-')[0];
          }
          const link = allLinks(node).find((l: any) =>
            integrationMiddleware.includes(l.source.id) ||
            integrationMiddleware.includes(l.target.id)
          );
          const mw = link
            ? integrationMiddleware.find((id: string) => id === link.source.id || id === link.target.id)
            : '';
          return (mw || '').split('-')[0] || '';
        };

        const bases = Array.from(
          new Set(integrationMiddleware.map((id: string) => id.split('-')[0]))
        ).sort();
        ...
        // Within same middleware base, MW-only above MW+direct-to-A
        const aDir = directToMain(a), bDir = directToMain(b);
        if (aDir !== bDir) return aDir ? 1 : -1;

        // Fallback: alphabetical by node type
    ...
    let { nodes, links } = sankeyGenerator(graph);

    // Sort links - middleware links first (matching original HTML)
    links = links.slice().sort((a: any, b: any) => {
      const isMWLink = (l: any) =>
        integrationMiddleware.includes(l.source.id) ||
        integrationMiddleware.includes(l.target.id);
    ...
    // Split links into direct and middleware (matching original HTML drawing order)
    const mwLinks = links.filter((l: any) =>
      integrationMiddleware.includes(l.source.id) ||
      integrationMiddleware.includes(l.target.id)
    );
    const directLinks = links.filter((l: any) => !mwLinks.includes(l));
    ...
    // Then draw middleware-attached links (on top)
    ...
    // Node labels - matching original HTML positioning
    node.append("text")
  ...
  }, [currentSystemCode, linksColorScale, nodesColorScale, handleNodeClick, handleNodeMouseOver, handleLinkMouseOver, handleMouseOut, processDataForSankey]);

  // Apply filters - matching original HTML filter logic exactly
  const applyFilters = useCallback(() => {
    if (!diagramData) return;

    const integrationMiddleware = diagramData.metadata.integrationMiddleware || [];
    const pinnedSystemCode = diagramData.metadata.code;

    // 1. Filter nodes based on node-specific criteria (focus nodes)
    ...
    // 3. Complex filtering logic matching original HTML
    const finalLinks: any[] = [];
    linkFilteredLinks.forEach(l => {
      if (((l.source === pinnedSystemCode && integrationMiddleware.includes(l.target)) || 
           (l.target === pinnedSystemCode && integrationMiddleware.includes(l.source))) && 
           (originalLinks.length === linkFilteredLinks.length) && 
           (originalNodes.length - 1 === focusNodes.length)) {
        // Skip middleware links when no filters are applied
      } else if (focusNodeIds.has(l.source) && integrationMiddleware.includes(l.target)) {
        // Focus node to middleware - add both focus->MW and MW->main links
        const middlewareLink = originalLinks.filter(obj => 
          obj.source === l.target && obj.target === pinnedSystemCode
        )[0];
        if (middlewareLink) finalLinks.push(middlewareLink);
        finalLinks.push(l);
      } else if (focusNodeIds.has(l.target) && integrationMiddleware.includes(l.source)) {
        // Middleware to focus node - add both main->MW and MW->focus links
        const middlewareLink = originalLinks.filter(obj => 
          obj.target === l.source && obj.source === pinnedSystemCode
        )[0];
        if (middlewareLink) finalLinks.push(middlewareLink);
        finalLinks.push(l);
      } else if (focusNodeIds.has(l.source) || focusNodeIds.has(l.target)) {
        // Direct links involving focus nodes
        finalLinks.push(l);
      }
    });

    // 4. Get final node IDs from links
  ...
  }, [diagramData, originalNodes, originalLinks, searchTerm, selectedType, selectedConnection, selectedCriticality, selectedFrequency, selectedRole, createSankey]);

  // Reset filters - matching original HTML
  ...
  // Get filter options - matching original HTML
  const getFilterOptions = useCallback(() => {
    ...
    return {
      types: ["All", ...new Set(originalNodes.map(d => d.type))],
      connections: ["All", ...new Set(originalLinks.map(d => d.pattern))],
      criticalities: ["All", ...new Set(originalNodes.map(d => d.criticality))],
      frequencies: ["All", ...new Set(originalLinks.map(d => d.frequency))],
      roles: ["All", "Producer", "Consumer"]
    };
  }, [originalNodes, originalLinks]);
  ...
  return (
    <div ref={containerRef} className="w-full h-full bg-gray-50 font-inter">
      <Card className="h-full bg-white shadow-lg rounded-xl">
        <CardHeader className="flex-row items-center justify-between space-y-0 pb-4">
          <CardTitle className="flex items-center text-3xl font-bold text-gray-800">
            ...
            {diagramData && `${diagramData.metadata.code} ${diagramData.metadata.review} Visualization`}
          </CardTitle>
        ...
        {diagramData && (
          <p className="text-gray-600 mt-1 px-6">
            Generated on: <span>{diagramData.metadata.generatedDate}</span>
          </p>
        )}
        ...
        <CardContent className="p-6">
          ...
          {diagramData && !isLoading && !error && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              {/* Controls Column - matching original HTML styling */}
              <div className="lg:col-span-3 space-y-4">
                <Card className="bg-white shadow-lg rounded-xl">
                  <CardHeader>
                    <CardTitle className="text-xl font-semibold border-b pb-2">Filters</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-6">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          ...
                          <Input
                            ...
                            className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg bg-white shadow-sm"
                          />
                        </label>
                      </div>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          ...
                          <DropDown
                            ...
                            options={filterOptions.frequencies.map(freq => ({ value: freq, label: freq }))}
                            className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg bg-white shadow-sm"
                          />
                        </label>
                      </div>
                    ...
                    <div className="border-t pt-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <h3 className="text-lg font-semibold mb-2 mt-4">Links Legend (Integration Pattern)</h3>
                        {[
              ...
              {/* Graph Column - matching original HTML styling */}
              <div className="lg:col-span-9">
                <Card className="bg-white shadow-lg rounded-xl overflow-hidden" style={{ height: '85vh' }}>
                  <CardContent className="p-0 w-full h-full relative overflow-scroll">
                    <svg
                      ref={svgRef}
      ...
      {/* Tooltip - matching original HTML styling */}
      <div
        ref={tooltipRef}
        className="absolute pointer-events-none opacity-0 bg-gray-800 text-white text-xs rounded-lg p-2 z-10 transition-opacity duration-200"
        style={{ 
          maxWidth: '300px',
