import React, { useState, useEffect } from 'react';
import { useParams } from 'react-router-dom';
import type { SankeyData, FilterState, Link } from '../../../../types/diagrams';
import { useFetchDiagramData } from '../../../../hooks/useFetchDiagramData';
import { useToast } from '../../../../context/ToastContext';
import SankeyFilters from './SpecificSystemFilters';
import SankeyDiagram from './SpecificSystemDiagram';
import SankeyLegend from './SpecificSystemLegend';

const SankeyVisualization: React.FC = () => {
  const { systemCode } = useParams<{ systemCode: string }>();
  const currentSystemCode = systemCode;

  // Data loading state
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [data, setData] = useState<SankeyData | null>(null);

  const [filters, setFilters] = useState<FilterState>({
    systemSearch: '',
    systemType: 'All',
    connectionType: 'All',
    criticality: 'All',
    frequency: 'All',
    role: 'All',
  });

  const [filteredData, setFilteredData] = useState<SankeyData | null>(null);

  // Hooks
  const { loadSystemFlows } = useFetchDiagramData();
  const { showError } = useToast();

  // Fetch data from backend
  const fetchDiagramData = async (systemCode: string) => {
    try {
      setIsLoading(true);
      setError(null);

      const diagramData = await loadSystemFlows(systemCode);
      setData(diagramData);
      setFilteredData(diagramData);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to load diagram data';
      setError(errorMessage);
      showError(errorMessage);
    } finally {
      setIsLoading(false);
    }
  };

  // Fetch data when systemCode changes
  useEffect(() => {
    if (currentSystemCode) {
      fetchDiagramData(currentSystemCode);
    }
  }, [currentSystemCode]);

  // Handle refresh
  const handleRefresh = () => {
    if (currentSystemCode) {
      fetchDiagramData(currentSystemCode);
    }
  };

  // const applyFilters = () => {
  //   if (!data) return;

  //   const searchTerm = filters.systemSearch.toLowerCase();
  //   const pinnedSystemId = data.metadata.code;
  //   const integrationMiddleware = data.metadata.integrationMiddleware || [];

  //   // Filter nodes based on node-specific criteria
  //   const focusNodes = data.nodes.filter(n => {
  //     const nameMatch = searchTerm === '' ||
  //       n.name.toLowerCase().includes(searchTerm) ||
  //       n.id.toLowerCase().includes(searchTerm);
  //     const typeMatch = filters.systemType === 'All' || n.type === filters.systemType;
  //     const criticalityMatch = filters.criticality === 'All' || n.criticality === filters.criticality;
  //     const roleMatch = filters.role === 'All' || n.id === pinnedSystemId 
  //     || (n.id.slice(-1) === filters.role.at(0));
  //     return nameMatch && typeMatch && criticalityMatch && roleMatch && n.id !== pinnedSystemId;
  //   });

  //   const focusNodeIds = new Set(focusNodes.map(n => n.id));

  //   // Filter links based on link-specific criteria
  //   const linkFilteredLinks = data.links.filter(l => {
  //     const connectionMatch = filters.connectionType === 'All' || l.pattern === filters.connectionType;
  //     const frequencyMatch = filters.frequency === 'All' || l.frequency === filters.frequency;
  //     return connectionMatch && frequencyMatch;
  //   });

  //   // Build final links including middleware connections
  //   const finalLinks: Link[] = [];
  //   linkFilteredLinks.forEach(l => {
  //     if (((l.source === pinnedSystemId && integrationMiddleware.includes(l.target)) ||
  //          (l.target === pinnedSystemId && integrationMiddleware.includes(l.source))) &&
  //         (data.links.length === linkFilteredLinks.length) &&
  //         (data.nodes.length - 1 === focusNodes.length)) {
  //       // Skip middleware links when showing all data
  //     } else if (focusNodeIds.has(l.source) && integrationMiddleware.includes(l.target)) {
  //       const middlewareLink = data.links.find(obj =>
  //         obj.source === l.target && obj.target === pinnedSystemId
  //       );
  //       if (middlewareLink) finalLinks.push(middlewareLink);
  //       finalLinks.push(l);
  //     } else if (focusNodeIds.has(l.target) && integrationMiddleware.includes(l.source)) {
  //       const middlewareLink = data.links.find(obj =>
  //         obj.target === l.source && obj.source === pinnedSystemId
  //       );
  //       if (middlewareLink) finalLinks.push(middlewareLink);
  //       finalLinks.push(l);
  //     } else if (focusNodeIds.has(l.source) || focusNodeIds.has(l.target)) {
  //       finalLinks.push(l);
  //     }
  //   });

  //   // Build final nodes
  //   const finalNodeIds = new Set<string>();
  //   finalLinks.forEach(l => {
  //     finalNodeIds.add(l.source);
  //     finalNodeIds.add(l.target);
  //   });
  //   finalNodeIds.add(pinnedSystemId);

  //   const finalNodes = data.nodes.filter(n => finalNodeIds.has(n.id));

  //   console.log(finalNodes);
  //   console.log(finalLinks);

  //   setFilteredData({
  //     nodes: finalNodes,
  //     links: finalLinks,
  //     metadata: data.metadata,
  //   });
  // };
  
  const applyFilters = () => {
  if (!data) return;

  const searchTerm = filters.systemSearch.toLowerCase();
  const pinnedSystemId = data.metadata.code;
  const integrationMiddleware = data.metadata.integrationMiddleware || [];

  // Filter nodes based on node-specific criteria
  const focusNodes = data.nodes.filter(n => {
    const nameMatch = searchTerm === '' ||
      n.name.toLowerCase().includes(searchTerm) ||
      n.id.toLowerCase().includes(searchTerm);
    const typeMatch = filters.systemType === 'All' || n.type === filters.systemType;
    const criticalityMatch = filters.criticality === 'All' || n.criticality === filters.criticality;
    const roleMatch = filters.role === 'All' || n.id === pinnedSystemId 
    || (n.id.slice(-1) === filters.role.at(0));
    return nameMatch && typeMatch && criticalityMatch && roleMatch && n.id !== pinnedSystemId;
  });

  const focusNodeIds = new Set(focusNodes.map(n => n.id));

  // Filter links based on link-specific criteria
  const linkFilteredLinks = data.links.filter(l => {
    const connectionMatch = filters.connectionType === 'All' || l.pattern === filters.connectionType;
    const frequencyMatch = filters.frequency === 'All' || l.frequency === filters.frequency;
    return connectionMatch && frequencyMatch;
  });

  // Build final links including middleware connections
  const finalLinks: Link[] = [];
  const addedLinks = new Set<string>(); // Track added links to prevent duplicates

  // Helper function to create unique link identifier using index for true uniqueness
  const getLinkId = (link: Link, index?: number) =>
    `${link.source}-${link.target}-${link.pattern || ''}-${link.frequency || ''}${index !== undefined ? `-${index}` : ''}`;

  linkFilteredLinks.forEach((l, index) => {
    const linkId = getLinkId(l, index);

    if (((l.source === pinnedSystemId && integrationMiddleware.includes(l.target)) ||
         (l.target === pinnedSystemId && integrationMiddleware.includes(l.source))) &&
        (data.links.length === linkFilteredLinks.length) &&
        (data.nodes.length - 1 === focusNodes.length)) {
      // Skip middleware links when showing all data
    } else if (focusNodeIds.has(l.source) && integrationMiddleware.includes(l.target)) {
      // Add the producer -> middleware link
      if (!addedLinks.has(linkId)) {
        finalLinks.push(l);
        addedLinks.add(linkId);
      }

      // Find ALL middleware -> main system links that match (not just the first one)
      const middlewareLinks = data.links.filter(obj =>
        obj.source === l.target && obj.target === pinnedSystemId
      );

      // For each matching middleware link, try to pair it
      if (middlewareLinks.length > 0) {
        // Find a middleware link that hasn't been added yet, or add all of them
        middlewareLinks.forEach((ml, mlIndex) => {
          const middlewareLinkId = getLinkId(ml, data.links.indexOf(ml));
          if (!addedLinks.has(middlewareLinkId)) {
            finalLinks.push(ml);
            addedLinks.add(middlewareLinkId);
            return; // Only add one per counterpart->middleware link
          }
        });
      }
    } else if (focusNodeIds.has(l.target) && integrationMiddleware.includes(l.source)) {
      // Add the consumer <- middleware link
      if (!addedLinks.has(linkId)) {
        finalLinks.push(l);
        addedLinks.add(linkId);
      }

      // Find ALL main system -> middleware links that match (not just the first one)
      const middlewareLinks = data.links.filter(obj =>
        obj.target === l.source && obj.source === pinnedSystemId
      );

      // For each matching middleware link, try to pair it
      if (middlewareLinks.length > 0) {
        // Find a middleware link that hasn't been added yet, or add all of them
        middlewareLinks.forEach((ml, mlIndex) => {
          const middlewareLinkId = getLinkId(ml, data.links.indexOf(ml));
          if (!addedLinks.has(middlewareLinkId)) {
            finalLinks.push(ml);
            addedLinks.add(middlewareLinkId);
            return; // Only add one per middleware->counterpart link
          }
        });
      }
    } else if (focusNodeIds.has(l.source) || focusNodeIds.has(l.target)) {
      // Direct connections (non-middleware)
      if (!addedLinks.has(linkId)) {
        finalLinks.push(l);
        addedLinks.add(linkId);
      }
    }
  });

  // Build final nodes
  const finalNodeIds = new Set<string>();
  finalLinks.forEach(l => {
    finalNodeIds.add(l.source);
    finalNodeIds.add(l.target);
  });
  finalNodeIds.add(pinnedSystemId);

  const finalNodes = data.nodes.filter(n => finalNodeIds.has(n.id));

  console.log('Final Nodes:', finalNodes);
  console.log('Final Links:', finalLinks);
  console.log('Added Links Count:', addedLinks.size);

  setFilteredData({
    nodes: finalNodes,
    links: finalLinks,
    metadata: data.metadata,
  });
};

  const resetFilters = () => {
    setFilters({
      systemSearch: '',
      systemType: 'All',
      connectionType: 'All',
      criticality: 'All',
      frequency: 'All',
      role: 'All',
    });
    if (data) {
      setFilteredData(data);
    }
  };

  // Loading state
  if (isLoading) {
    return (
      <div className="bg-gray-100 min-h-screen font-inter text-gray-800">
        <div className="max-w-screen-2xl mx-auto p-4 lg:p-6">
          <div className="flex items-center justify-center h-96">
            <div className="flex items-center space-x-2">
              <svg className="animate-spin h-8 w-8 text-gray-400" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 714 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
              <span className="text-gray-500 text-lg">Loading system flow diagram...</span>
            </div>
          </div>
        </div>
      </div>
    );
  }

  // Error state
  if (error || !data) {
    return (
      <div className="bg-gray-100 min-h-screen font-inter text-gray-800">
        <div className="max-w-screen-2xl mx-auto p-4 lg:p-6">
          <div className="flex items-center justify-center h-96">
            <div className="text-center">
              <svg className="w-16 h-16 text-red-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-red-600 font-medium text-lg">Failed to load diagram</p>
              <p className="text-gray-500 text-sm mt-1">{error || 'No data available'}</p>
              <button
                onClick={handleRefresh}
                className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
              >
                Try Again
              </button>
            </div>
          </div>
        </div>
      </div>
    );
  }

  // No system code provided
  if (!currentSystemCode) {
    return (
      <div className="bg-gray-100 min-h-screen font-inter text-gray-800">
        <div className="max-w-screen-2xl mx-auto p-4 lg:p-6">
          <div className="text-center py-8">
            <p className="text-gray-500 text-lg">No system code provided</p>
          </div>
        </div>
      </div>
    );
  }

  const pinnedSystemId = data.metadata.code;
  const integrationMiddleware = data.metadata.integrationMiddleware || [];

  return (
    <div className="bg-gray-100 min-h-screen font-inter text-gray-800">
      <div className="max-w-screen-2xl mx-auto p-4 lg:p-6">
        {/* Header */}
        <header className="mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-800">
                {pinnedSystemId} {data.metadata.review} Visualisation
              </h1>
              <p className="text-gray-600 mt-1">
                Generated on: <span>{data.metadata.generatedDate}</span>
              </p>
            </div>
            <button
              onClick={handleRefresh}
              disabled={isLoading}
              className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center space-x-2"
              title="Refresh diagram"
            >
              <svg className={`w-4 h-4 ${isLoading ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
              <span>Refresh</span>
            </button>
          </div>
        </header>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Filters Column */}
          <div className="lg:col-span-3">
            <SankeyFilters
              filters={filters}
              onFiltersChange={setFilters}
              onApplyFilters={applyFilters}
              onResetFilters={resetFilters}
              originalNodes={data.nodes}
              originalLinks={data.links}
              pinnedSystemId={pinnedSystemId}
            />
            <SankeyLegend />
          </div>

          {/* Graph Column */}
          <div className="lg:col-span-9">
            <div className="bg-white rounded-xl shadow-lg overflow-hidden" style={{ height: '85vh' }}>
              <div className="w-full h-full relative overflow-auto">
                {filteredData && (
                  <SankeyDiagram
                    data={filteredData}
                    width={1000}
                    height={1000}
                    pinnedSystemId={pinnedSystemId}
                    integrationMiddleware={integrationMiddleware}
                  />
                )}
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default SankeyVisualization;