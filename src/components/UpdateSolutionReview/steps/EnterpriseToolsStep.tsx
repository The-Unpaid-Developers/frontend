...
const empty: EnterpriseTool = {
  tool: { name: '', type: '', }, onboardingStatus: '', integrationDetails: '', issues: ''
};
...
const EnterpriseToolsStep: React.FC<StepProps> = ({ onSave, isSaving = false, initialData }) => {
  ...
  const updateTool = <K extends keyof EnterpriseTool['tool']>(k: K, v: EnterpriseTool['tool'][K]) =>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        <Input placeholder="Tool Name" value={row.tool.name} onChange={e => updateTool('name', e.target.value)} />
            ...
            <tbody>
              {list.map((t, i) => (
                <tr key={i} className="border-t">
                  ...
                  <td className="p-2">{t.onboardingStatus}</td>
